...
public class BatchComparator implements Comparator<Batch> {
  ...
  public int compare(Batch _this, Batch other) {
    if (collator.compare(_this.getThisProductID(), other.getThisProductID()) == 0) {
      if (collator.compare(_this.get_partner(), other.get_partner()) == 0) {
        if (_this.getPrice() - other.getPrice() == 0)
          return _this.getStock() - other.getStock();
        return (int) _this.getPrice() - (int) other.getPrice();
      }
      return collator.compare(_this.get_partner(), other.get_partner());
    }
    return collator.compare(_this.getThisProductID(), other.getThisProductID());
  }
